class TransactionsController < ApplicationController
  ...
  def transaction_params
    #params.permit(:postal_code, :prefecture_id, :city, :house_number, :building_name, :phone_number).merge(user_id: current_user.id).merge(item_id: @item.id).merge(price: @item.price).merge(token: params[:token])
